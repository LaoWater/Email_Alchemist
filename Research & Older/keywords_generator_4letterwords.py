...
def create_table(cur):
    ...
    cur.execute('DROP TABLE IF EXISTS 4letters_words')

    # Create table for storing valid 3-letter words
    cur.execute('''
    CREATE TABLE IF NOT EXISTS 4letters_words (
...
def filter_and_store_english_words(cur, valid_words):
    ...
    for word in valid_words:
        if len(word) == 4 and is_english(word):
            count += 1
            print(f"Valid Word found! {word} at count {count}")

            # Insert the valid word and count into the database
            cur.execute("INSERT INTO 4letters_words (word, count) VALUES (%s, %s)", (word, count))
